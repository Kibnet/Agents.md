...
- Обязательный набор проверок, условия full suite и regression check выбирать по `testing-baseline`; сохранять явные repo/CI/release gates и проверки утверждённой SPEC.
- После изменений выполнять затронутую сборку и выбранный обязательный набор с тем runner workflow, который принят в проекте. Для заявления об обычной сборке проверить стандартную команду пользователя без скрывающих проблему диагностических исключений.
...
- Локально сначала выполнять targeted tests, затем остальные обязательные проверки; full-run выполнять при условиях его owner.
...
- Для known slow suite использовать repo-specific serial/progress options, не ослабляя выбранные обязательные gates.
