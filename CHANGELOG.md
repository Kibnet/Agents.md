...
- Routing сначала определяет применимость и специальные условия owner, затем сравнивает строгость одновременно применимых требований. Общая иерархия инструкций и применимые consumer MUST сохраняются.
- Collaboration сохраняет исходную цель, фазу и уже полученные полномочия между ходами. Конкретное «да» принимается в его scope; обратимые внутренние решения не требуют нового вопроса. Новые материал, адресат или существенный production-риск проверяются отдельно.
- Short SPEC и review используют пять содержательных проверок без обязательных 20 отдельных оценок, шести числовых баллов и неприменимых ролей. Expanded/high-risk сохраняет глубокий review. Журнал фиксирует фазы, решения, блокеры и завершение.
- Review findings имеют одного owner. Невыполненные обязательные AC, authorization и validation блокируют PASS независимо от severity; MEDIUM получает disposition по влиянию на контракт, LOW вне scope не расширяет задачу автоматически.
- Testing выбирает обязательный набор до реализации по области влияния; для ограниченных UI/copy/layout изменений предусмотрена адресная проверка. Широкий/неизвестный риск, общие контракты и явные repo gates требуют full suite.
- Completion проверяет исходный сценарий, обычную сборку/запуск и действительно просмотренное визуальное состояние. Claim о persistence отделён от сохранения настройки. Источники, язык и детализация артефакта сохраняются; массовый охват отделён от полезного продвижения.

### Added

- Узкие static contract checks и безопасный paired behavioral smoke по синтетическим сценариям. Фактические результаты и ограничения фиксируются в [evidence](specs/evidence/2026-09-20-outcome-and-friction-improvements.md).

### Breaking / Migration

- MAJOR повышен из-за изменения порядка разрешения норм и обязательной формы validation/review. Перед применением проверить consumer overrides и их конкретные условия; прежние approved SPEC obligations и явные repo/CI/release gates не снимаются автоматически.
- Exact «Спеку подтверждаю», границы внешних действий, runtime enforcement, high-risk review и mandatory instruction smoke сохранены. Config, модели, hooks, skills и память этим изменением не обновляются.

### Rollback

- Применять проверенный isolated candidate одним allowlisted change set после сверки HEAD/dirty/preimage hashes; повторить critical checks на активном пути. Откатить только затронутые файлы с совпадающим postimage, сохраняя параллельные изменения. Возврат каталога не отменяет уже выполненные внешние действия и не меняет runtime/config.
