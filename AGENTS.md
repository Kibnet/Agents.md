...
6. Локальный `AGENTS.override.md` в репозитории-потребителе применять только после central stack как дополнительные локальные инструкции поверх него; он не заменяет центральный `AGENTS.md` и может только ужесточать центральные MUST.
7. Для STORM product workflow, BDD/Gherkin behavior layer и команд `/storm:*` использовать [instructions/profiles/storm-product-development.md](instructions/profiles/storm-product-development.md).
8. Для OpenAI Responses API, exact model/tier routing, persisted reasoning, Programmatic Tool Calling и Responses multi-agent использовать [instructions/governance/openai-responses-api.md](instructions/governance/openai-responses-api.md) только по соответствующему триггеру.
...
2. Открыть `instructions/governance/routing-matrix.md` и до классификации применить `creator-vibe-lens` как lightweight owner.
3. Классифицировать задачу и собрать instruction stack, включая `model-behavior-baseline` и, для `tool-heavy` задач, `tool-execution-baseline`; зафиксировать фактическую surface/runtime конфигурацию, если она влияет на результат, и разрешать конфликты только по алгоритму матрицы.
4. Если creative/human-experience trigger сработал и полный `creator-vibe` доступен, загрузить его до более узких skills/profiles.
5. Если в consumer-репозитории есть `AGENTS.override.md`, применить его после central stack как дополнительные локальные инструкции поверх него.
6. Если пользователь вызывает `/storm:*`, подключить профиль `storm-product-development`; команды с изменениями tests/code/behavior должны идти через обычный `delivery-task` и `QUEST`, включая `/storm:bdd-implement`.
